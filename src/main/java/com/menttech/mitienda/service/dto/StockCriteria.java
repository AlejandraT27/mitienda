package com.menttech.mitienda.service.dto;

import java.io.Serializable;
import java.util.Objects;
import io.github.jhipster.service.Criteria;
import io.github.jhipster.service.filter.BooleanFilter;
import io.github.jhipster.service.filter.DoubleFilter;
import io.github.jhipster.service.filter.Filter;
import io.github.jhipster.service.filter.FloatFilter;
import io.github.jhipster.service.filter.IntegerFilter;
import io.github.jhipster.service.filter.LongFilter;
import io.github.jhipster.service.filter.StringFilter;

/**
 * Criteria class for the {@link com.menttech.mitienda.domain.Stock} entity. This class is used
 * in {@link com.menttech.mitienda.web.rest.StockResource} to receive all the possible filtering options from
 * the Http GET request parameters.
 * For example the following could be a valid request:
 * {@code /stocks?id.greaterThan=5&attr1.contains=something&attr2.specified=false}
 * As Spring is unable to properly convert the types, unless specific {@link Filter} class are used, we need to use
 * fix type specific filters.
 */
public class StockCriteria implements Serializable, Criteria {

    private static final long serialVersionUID = 1L;

    private LongFilter id;

    private StringFilter numberProduct;

    private StringFilter nameProduct;

    private StringFilter description;

    private StringFilter inventory;

    public StockCriteria() {
    }

    public StockCriteria(StockCriteria other) {
        this.id = other.id == null ? null : other.id.copy();
        this.numberProduct = other.numberProduct == null ? null : other.numberProduct.copy();
        this.nameProduct = other.nameProduct == null ? null : other.nameProduct.copy();
        this.description = other.description == null ? null : other.description.copy();
        this.inventory = other.inventory == null ? null : other.inventory.copy();
    }

    @Override
    public StockCriteria copy() {
        return new StockCriteria(this);
    }

    public LongFilter getId() {
        return id;
    }

    public void setId(LongFilter id) {
        this.id = id;
    }

    public StringFilter getNumberProduct() {
        return numberProduct;
    }

    public void setNumberProduct(StringFilter numberProduct) {
        this.numberProduct = numberProduct;
    }

    public StringFilter getNameProduct() {
        return nameProduct;
    }

    public void setNameProduct(StringFilter nameProduct) {
        this.nameProduct = nameProduct;
    }

    public StringFilter getDescription() {
        return description;
    }

    public void setDescription(StringFilter description) {
        this.description = description;
    }

    public StringFilter getInventory() {
        return inventory;
    }

    public void setInventory(StringFilter inventory) {
        this.inventory = inventory;
    }


    @Override
    public boolean equals(Object o) {
        if (this == o) {
            return true;
        }
        if (o == null || getClass() != o.getClass()) {
            return false;
        }
        final StockCriteria that = (StockCriteria) o;
        return
            Objects.equals(id, that.id) &&
            Objects.equals(numberProduct, that.numberProduct) &&
            Objects.equals(nameProduct, that.nameProduct) &&
            Objects.equals(description, that.description) &&
            Objects.equals(inventory, that.inventory);
    }

    @Override
    public int hashCode() {
        return Objects.hash(
        id,
        numberProduct,
        nameProduct,
        description,
        inventory
        );
    }

    @Override
    public String toString() {
        return "StockCriteria{" +
                (id != null ? "id=" + id + ", " : "") +
                (numberProduct != null ? "numberProduct=" + numberProduct + ", " : "") +
                (nameProduct != null ? "nameProduct=" + nameProduct + ", " : "") +
                (description != null ? "description=" + description + ", " : "") +
                (inventory != null ? "inventory=" + inventory + ", " : "") +
            "}";
    }

}
